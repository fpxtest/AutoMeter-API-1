...
@Component
public class CheckSlaverAliveScheduleTask {
        ...
        try {
            ...
            if (lock) {
                try {
                    ...
                    CheckSlaverAliveScheduleTask.log.info("调度服务检查Slaver是否Alive定时器异常=======================" + ex.getMessage());
                } finally {
                    //TODO 执行任务结束后需要释放锁
                    //释放锁
                    redisUtils.deletekey(redisKey);
                    CheckSlaverAliveScheduleTask.log.info("调度服务检查Slaver是否Alive定时器-============释放分布式锁成功=======================");
                }
            } else {
                CheckSlaverAliveScheduleTask.log.info("调度服务检查Slaver是否Alive定时器-============{}机器上占用分布式锁，正在执行中=======================" + redisKey);
                return;
            }
        } catch (Exception ex) {
            CheckSlaverAliveScheduleTask.log.info("调度服务检查Slaver是否Alive定时器-异常: " + ex.getMessage());
        }
    }

    public void CheckAliveSlaver() {
        List<Slaver> SlaverList=slaverMapper.selectAll();
        for (Slaver slaver : SlaverList) {
            ...
            try {
                ...
                CheckSlaverAliveScheduleTask.log.info("检测：" + ServerUrl + "请求响应结果。。。。。。。。。。。。。。。。。。。。。。。。：" + respon);
            } catch (Exception e) {
                //1.置为已下线
                slaverMapper.updateSlaverStatus(slaver.getId(), "已下线");
                CheckSlaverAliveScheduleTask.log.info(slaver.getSlavername()+"检测结果为下线：" + ServerUrl + "请求响应结果异常。。。。。。。。。。。。。。。。。。。。。。。。：" + e.getMessage());
                //补偿
                if(slaver.getStype().equalsIgnoreCase("功能"))
                {
                    ...
                    for (String ids:PlanidBatchidmap.keySet()) {
                        ...
                        CompensateAfterFail("用例所分配的执行机:"+slaver.getSlavername()+"已下线，请检查此slaver是否正常运行！", PlanidBatchidmap.get(ids),Planid,dispatchList);
                    }
                }
                else
                {
    ...
    public List<Slaver> GetAllAliveSlaver() {
        ...
        for (Slaver slaver : slaverlist) {
            ...
            try {
                ...
                CheckSlaverAliveScheduleTask.log.info("检测GetAliveSlaver：" + ServerUrl + "请求响应结果。。。。。。。。。。。。。。。。。。。。。。。。：" + respon);
            } catch (Exception e) {
                CheckSlaverAliveScheduleTask.log.info("检测GetAliveSlaver：" + ServerUrl + "请求响应结果。。。。。。。。。。。。。。。。。。。。。。。。：" + e.getMessage());
                slaverMapper.updateSlaverStatus(slaver.getId(), "已下线");
            }
        }
    ...
    @PostConstruct
    public void Init() {
        redisKey = "CheckSlaverAliveScheduleTask-RedisLock" + new Date();
        CheckSlaverAliveScheduleTask.log.info("调度服务检查Slaver是否Alive定时器-redisKey is:" + redisKey);
    }
}
