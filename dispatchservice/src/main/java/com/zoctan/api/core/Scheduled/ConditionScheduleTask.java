...
public class ConditionScheduleTask {
            ...
            if (lock) {
                ...
                if (dispatch != null) {
                    try {
                        ...
                        testcondition.createCriteria().andCondition("objectid = " + PlanID)
                                .andCondition("objecttype = '执行计划'");
                        if (testconditionService.ifexist(testcondition) > 0) {
                            ConditionScheduleTask.log.info("调度有条件需要处理................................");
                            //判断planid,batchid在conditionreport表中无记录，才执行，避免多次重复调用
                            Condition reportcon = new Condition(TestconditionReport.class);
                            reportcon.createCriteria().andCondition("testplanid = " + PlanID)
    ...
    private void RequestConditionServiceByPlanId(Dispatch dispatch) throws Exception {
        ...
        ConditionScheduleTask.log.info("调度处理条件任务请求conditionserver。。。。。。。: " + conditionserver);
        try {
